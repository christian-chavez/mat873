...
\right\}
\]
\red{illustrar el llenado de un polygono graficamente}
...
tels que 
si \(\sigma\in A\) et \(\tau\subset \sigma\) est nonvide,
alors 
...
\begin{ex}
\begin{enumerate}[label=(\alph*)]
\item Les ensembles \(A = \{\{a\}, \{b\} \}\) et \(B = \{\{\{a\}\}, \{b \}\}\) sont des  complexe simpliciaux. L'ensemble  \(\{a, \{b \} \}\) n'est pas un complexe simplicial.
\item Cet exemple montre que les étiques ne sont pas importants : 
\[A = \{\{\clubsuit , \bigstar  \}, \{\blacklozenge , \spadesuit\}, \{\clubsuit\}, \{\bigstar \}, \{\blacklozenge \}, \{\spadesuit \}\}\] 
est bien un complexe simplicial.
\item \(A = \{\{\varnothing\}\}\) est un complex. En fait, \(\{\{x\}\}\) est un complex pour tout objet \(x\). Est-ce que \(\{\{\{x\}\}\}\) est un complex ?
\item Un graphe est un example d'un complexe simplicial abstrait, 
...
Voici un exmple ``pas trivial''
\begin{ex}\label{exp:triangle-arete}
L'ensemble
\begin{align*}
A 
&= \left\{ 
...
On a écrit tous les sous ensembles nonvides de \(\left\{ 0,1,2 \right\}\) dans la premiere ligne,
et tous les sous ensembles nonvides de \(\left\{ 2,3 \right\}\) dans la deuxième, (avec pas répetition).
\end{ex}

On remarque que l'écriture n'est pas si efficient,
donc on utilizerá une notation plus prátique.
...
Équivalentement, \red{?}
\[
[a_0, \dots, a_n] = \mathcal{P}(\left\{ a_0, \dots, a_n \right\}) \setminus \left\{ \varnothing \right\}.
\]
Avec cette notation le complexe de l'exemple \autoref{exp:triangle-arete} devient 
\[
A = [ 0,1,2] \cup [2,3]
\]
où 
...
\left\{ 
    ...
    \left\{ 3 \right\}
\right\},\\
[2,3] &= \left\{     \left\{ 2,3 \right\}, \left\{ 2 \right\},
    \left\{ 3 \right\} \right\}.
\end{align*}

Or, on voudrait utiliser  encore une notation plus efficace.

\red{Benv notes: they omit the parethenses and commas altogether ``Lorsqu’il n’y a pas de confusion possible, on omet les parenthèses et les virgules pour les sim-
plexes, par exemple le simplexe {1, 2, 3} est simplement noté 123''}
...
on se permet d’omettre de mentionner la présence de certains simplexes lorsque leur présence est assurée par
celle d’un autre simplexe dont c’est la face. Par exemple, si on déclare que C = {1234, 45}, ça signifie que
